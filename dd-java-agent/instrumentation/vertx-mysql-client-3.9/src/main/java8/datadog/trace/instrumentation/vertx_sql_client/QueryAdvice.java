  ...
  public static class Execute {
    ...
    public static <T, R extends SqlResult<T>> AgentScope beforeExecute(
      ...
      final TraceScope.Continuation continuation =
          null == currentSpan ? null : captureSpan(currentSpan);
      if (prepared) {
        handler = new QueryResultHandlerWrapper<>(handler, clientSpan, continuation);
      } else {
        maybeHandler =
            new QueryResultHandlerWrapper<>(
                (Handler<AsyncResult<R>>) maybeHandler, clientSpan, continuation);
      }
